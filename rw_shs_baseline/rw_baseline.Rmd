...
wetChem <- read.csv(paste("/Users/mlowes/Google Drive/jupyter/Rwanda Acidity", "Original chem_rwanda_shs.csv", sep = "/"))
```
...
```{r}
d %>%
  dplyr::select(gps_pic_guid, pH, Total.N, Total.C, district, cellule_field, umudugudu, aez, cell, lat, lon, alt, precision, black_soil, red_soil, sandy_soil) %>%
  write.csv(., file = "data/rwanda_soil_data_for_map.csv", row.names = F)
```
...
crdref <- CRS('+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')
e <- d[!is.na(d$lon),] # where d is the data I've shared.
ss <- SpatialPointsDataFrame(coords = e[, c("lon", "lat")], data=e, proj4string = crdref)
