...
attritionTab <- d %>% 
  ...
  mutate(sample_id = tolower(sample_id)) %>%
  filter(sample_id %in% attOut$sample_id)

attritionTab <- attritionTab[,c(4,1,2,3,5)]

write.csv(attritionTab, file=paste0("attritionTabs/attrition table", format(Sys.time(), '%Y %m %d')))
