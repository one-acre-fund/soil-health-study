...
* reality check key agronomic values
* summarize odd values by enumerator
* look for high NA rates in variables
...
iqr.check <- function(dat, x) { 
  q1 = summary(dat[,x])[[2]]
  q3 = ifelse(exists(summary(dat[,x])[[5]]), summary(dat[,x])[[5]], 0) 
  iqr = q3-q1
...
# remove admin vars
numAdminVars <- c("metadata.deviceID", "oafid", "X", "found", "")
