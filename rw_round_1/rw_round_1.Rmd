...
```{r}
baselineDir <- normalizePath(file.path("..", "rw_shs_baseline", "data"))
...
10/5/18 - I had to download the files and add them to the repo to streamline getting the data I needed. When syncing is complete I should be able to draw from the 1AF Soil Lab folder.
...
soil <- read.csv("combined-predictions-including-bad-ones.csv")
...
Identifiers <- read_excel("database.xlsx", sheet = 1)
```
...
```{r}
rawDir <- normalizePath(file.path("..", "rw_shs_round_1_master", "Soil health study (year one)", "data"))
...
## Save data 
For importing to shs_analysis to update the cleaning file and update the analysis overall.

Save the Rwanda R1 data to include it in the `shs_analysis/shs_cleaning.R` work.

```{r}
saveRDS(fieldDat, file = "r1FieldDat.rds")
```
