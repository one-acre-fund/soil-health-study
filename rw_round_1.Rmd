...
```{r}
commonVars <- names(rs)[names(rs) %in% names(bUpdate)] # using rs because i changed baseline names to rs names
...
```{r}
#table(fieldDat$d_client, fieldDat$season)
fieldDat %>% 
...
* Client Y0 vs. client Y1
* Client Y1 vs. non-client Y1
...
* Individual fixed effects account for things specicic to farmeer that don't change over time
...
```{r}
indFe <- do.call(rbind, lapply(keySoilVars, function(x){
  mod = plm(lm(as.formula(fieldSoilDat[,x] ~ as.factor(d_client) + as.factor(sample_id) + as.factor(season)),data=fieldSoilDat), c("sample_id", "season"))
  #mod = lm(as.formula(sameRegBr[,x] ~ as.factor(season)), data=sameRegBr)
  #mod.cluster <- coeftest(mod, clust.se(mod, sameRegBr$sample_id))
  
  rownames(mod) = paste(rownames(mod), x, sep = " ")
  
  return(mod)
}))
write.csv(indFe, file="output/indFe.csv")
