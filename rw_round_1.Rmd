...
#sameStatus <- soilDat[soilDat$sample_id %in% sameStatusVec$sample_id,] #all appear twice
...
baseVr1Mod <- do.call(rbind, lapply(keySoilVars, function(x){
  mod = plm(lm(as.formula(sameRegBr[,x] ~ as.factor(season) + as.factor(sample_id)), data=sameRegBr), "sample_id")
