...
fieldDat %>% 
  ...
  dplyr::summarize_each(
    funs(mean(., na.rm=T)), -c(sample_id, client15b, client16a, client16b)
  ) %>% 
  mutate_each(
    funs(paste0(round(.,2)*100, "%"))
  ) %>%
  kable(format='markdown', caption="Movement in Sample")
...
Table comparing the values seen above with a p-value

**Note**: This table is preliminary and does not reflect values ready for interpretation (6/19)
