...
bUpdate <- b %>% 
  ...
  X.Exchangeable.Aluminum = Exchangeable.Aluminium
)

# check what's already the same
matchNames <- names(rs)[names(rs) %in% names(bUpdate)]
# matchNames

unmatchedB <- names(bUpdate)[!names(bUpdate) %in% names(rs)]
unmatchedRs <- names(rs)[!names(rs) %in% names(bUpdate)]
