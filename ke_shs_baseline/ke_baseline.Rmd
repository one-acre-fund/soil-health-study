...
**03/26/2019 I'VE CHANGED THE SOURCE OF THE SOILDIR FROM 'DATA' TO MISC_SOIL_DATA ON GOOGLE DRIVE. THIS MIGHT HAVE BROKEN SOME LINKS IN OTHER SCRIPTS AS WELL. I'VE UPDATED IT HERE BUT YOU MIGHT NEED TO UPDATE OTHER FOLDERS!**

```{r import}
# wd <- "/Users/mlowes/Google Drive /optimized_agronomy/soil/soil_health_study/data/ke baseline"
# dd <- paste(wd, "data", sep = "/")
# od <- paste(wd, 'output', sep = "/")
dd <- normalizePath(file.path("ke_shs_baseline", "data"))
soilDir <- normalizePath(file.path("..", "..", "general_data_folder", "soil", "predictions"))

od <- normalizePath(file.path("ke_shs_baseline", "output"))

load(paste("/Users/mlowes/Google Drive/r_help/3_analyze/regressions - general, R", "regression_functions.Rdata", sep = "/"))
...
cnP <- read.csv(paste(soilDir, "Kenya_SHS_N_C.csv", sep = "/"))
Identifiers <- read.csv(paste(soilDir, "Combined meta with SSN.csv", sep = '/'), stringsAsFactors = F)
```
...
## Export `d` for R1 follow up
```{r}
export <- d[,c("oaf", "id.RegionName", "id.DistrictName", "id.SiteName", "id.Last_Name", "id.First_Name","id.OAFID", "id.ClientPhone", "field_location", "SSN")]

names(export) <- c("group", "Province", "District", "Site", "Farmer last name", "Farmer first name", "oafid", "Client Phone", "Field Location", "hidden.ssn")

export[,grep("name", names(export))] <- sapply(export[,grep("name", names(export))], tolower)
write.csv(export, file=paste(od, "r1 client list export.csv", sep = "/"), row.names=F)
```


## Cleaning baseline variables
```{r}
# drop obs without SSN
d <- d[!is.na(d$SSN),]
...
## Output control sheet for R3
```{r}
baselineCheck <- d %>%
  mutate(sample_id = tolower(gsub(" ", "", Soil_Sample_Id)))


library(googlesheets)

samoraSheet <- gs_title("Control Sheet Soil Sample 2017.xls") %>% gs_read(ws = "Sheet1") %>%
  mutate(unique.id = tolower(gsub(" ", "", UNIQUEID)))

# check that all the ids in the baseline data are in Samora's control sheet
table(samoraSheet$unique.id %in% baselineCheck$sample_id)

# look at missing ones
samoraSheet$unique.id[!samoraSheet$unique.id %in% tolower(d$Soil_Sample_Id)] %>%
  write.csv(., file = paste(od, "extra_obs_in_control_sheet.csv", sep = "/"))


# and the other way << what from baseline is not in Samora's sheet
# everything is there.
table(baselineCheck$sample_id %in% samoraSheet$unique.id)
...
### Import wet chem for CEC summary
I pulled the wet chem data from Emmanuel's Project soil data folder/Final soil datasets/Kenya/Kenya_SHS/Kenya_SHS_RF/Wetchem

```{r}
wetChem <- read.csv(paste(dd, "Kenya_SHS_wetchem.csv", sep = "/"), stringsAsFactors = F)
ggplot(wetChem, aes(x=C.E.C)) + geom_density() + 
  geom_vline(xintercept = median(wetChem$C.E.C, na.rm=T), color="red") + 
  geom_vline(xintercept = mean(wetChem$C.E.C, na.rm=T), color="blue")

summary(wetChem$C.E.C)[3:4]
```
...
These data checks are happening in March, 2019 trying to make sense of results we've seen after a couple years of soil health study work. Specifically, I want to know N levels baesd on the years of previous fertilizer cultivation

```{r}
d %>%
  filter(!is.na(chemical_fertilizer_5)) %>%
  ggplot(., aes(x = as.factor(chemical_fertilizer_5), y = Total.N, fill = as.factor(oaf))) +
  geom_boxplot() +
  labs(title = "Nitrogen levels by previous years of chemical fertilizer",
       subtitle = "We don't see a big drop off due to increasing fertilizer use",
       fill = "Current 1AF status", x = "Chemical fertilizer use in previous 5 seasons", y = "Total N (%)")
```

Looking at baseline N rates by chemical fertilizer use for the past 5 years suggests that repeated cultivation isn't driving down N rates dispropportionally among 1AF field. Instead we see that if anything fields that have had 5 years of chemical fertilizer use in the past 5 seasons and are currently 1AF plots have slightly higher N averages than controls.

```{r}
d %>%
  filter(!is.na(chemical_fertilizer_5)) %>%
  group_by(chemical_fertilizer_5, oaf) %>%
  summarize(count = n()) %>%
  spread(oaf, count) %>%
  kable(., format = 'markdown')
```
...
These were the days before Matt understood how to use a function...

## Table 7 

```{r}
table7vars <- c("pH", "Total.C", "Total.N", "m3.Ca", "m3.Mg")
table7 <- output[rownames(output) %in% table7vars, ]

write.csv(table7, file=paste(od, "ke table7.csv", sep = "/"), row.names=T)
...
[Guide to interpreting Linear-Log Models](http://www-stat.wharton.upenn.edu/~stine/stat621/handouts/LogsInRegression.pdf)
...
Only keep non-zero compost farmers for the regression

```{r}
previousSeasoncompost <- paste("compost.acre", "as.factor(SiteName)", sep=" + ")
temp <- subset(d, d$compost.acre>0)
list.t18b <- lapply(soilVars, function(x){
  mod <- lm(as.formula(paste("temp[, x] ~", previousSeasoncompost, sep="")),
            data=temp)
  return(mod)
})
...
```{r}
soilC.compost <- ggplot(temp, aes(x = compost.acre, y=Total.C)) + geom_point() + 
  scale_x_continuous(limits=c(0,50)) + # remove larger values from the graph.
  geom_smooth(method='lm') + 
  labs(title = "Compost and Soil Carbon in Kenya", x = "Compost (kg/acre)", y = "Total Carbon")

soilC.compost

pdf(paste(od, "compost vs soil c.pdf", sep = "/"), width=11, height=8.5)
print(soilC.compost)
dev.off()
```
...
```{r}
write.csv(equal, file=paste(od, "ke female farmer status.csv", sep = "/"), row.names=T)
```


## Export the data for Eric
```{r}
write.csv(d, file=paste(od, "kenya_shs_merged_clean.csv",sep = "/"), row.names=F)
```
...
```{r}
# Change colnames of MeanT and MeanC to Client and New Client
...
write.csv(thresh, file=paste(od, "table1_ke_thresholds.csv", sep = "/"), row.names = T)
```

Clients vs. Non-clients summary table
...
newOld <- d %>% filter(!is.na(tenure.tiers)) %>% group_by(tenure.tiers) %>% 
  ...
  calcium = round(mean(Ca, na.rm=T),3),
  magnesium = round(mean(Mg, na.rm=T),3)
) %>% ungroup() 
...
New Clients vs. Returning Non-clients threshold table
...
colnames(thresh.t) = c("new client", "returning client")
thresh.t = thresh.t[-1, ]

thresh.t <- cbind(newOld, thresh.t)

write.csv(thresh.t, file=paste(od, "table1_ke_thresholds_tenure.csv", sep = "/"), row.names = T)
```
...
**Tip**: [Zoom in on subset of full map](http://stackoverflow.com/questions/18323832/plot-small-region-of-a-large-polygon-map-in-ggplot2) and [filling polygons with base R plot](https://rpubs.com/lmullen/46313)

I made the the map but there are so many observations in that one polygon. Figure out why and update the map!
...
Look at all polygons with a positive survey count as well as the one with the most, Kanduyi.

```{r}
unique(plotCount@data[!is.na(plotCount@data$count),]$NAME_2)

plotCount@data[which.max(plotCount@data$count),]
```

Now work out which GPS points are assocated with that polygon and look at how many districts and sites it actually represents. Keep only the ones that should be in Kanduyi and drop the rest from this map. I suspect that GPS were incorrectly assigned to the office and thus show as being in Kanduyi when in fact they should be elsewhere!

```{r}
office <- subset(ss, ss$spatialname=="Kanduyi")
table(office$DistrictName)
table(office$SiteName)
```

Based on the districts and sites associated with the Kanduyi polygon, are there any we would reassign? I don't know the district boundaries well enough. One option is to map only Kanduyi and see how many points are concentrated in a particular spot as opposed to on the boarders.

```{r}
plot(office)
```

This is a simple map but we see that some of points are correctly assigned while most are concentrated in a single spot. Let's look at the distribution of lat and lon and pull out the outliers

```{r}
ggplot(office@data, aes(x=lon, y=lat)) + geom_point()
```

Keep the points between  34.55 lon and less than 34.6 lon

```{r}
office <- subset(office, subset=office$lon >34.55 & office$lon<34.6)
```

And now see where these points are supposed to be:
```{r}
table(office$DistrictName)
length(office)
```

At least 159 samples should be redistributed to other sites. 

## Re-assigning GPS centered at the office

**Note**: I don't have a good way to find these points again in the object ke, reassign the values and generate new values. Instead, I can subtract values from Kanduyi and try to assign them elsewhere manually. See if this is possible.

```{r}
ss.count[ss.count$spatialname=="Butere", "count"] <- ss.count$count[ss.count$spatialname=="Butere"] + 12

ss.count[ss.count$spatialname=="Lugari", "count"] <- ss.count$count[ss.count$spatialname=="Lugari"] + 57

# and subtract from kanduyi
ss.count[ss.count$spatialname=="Kanduyi", "count"] <- ss.count$count[ss.count$spatialname=="Kanduyi"] - 159
```
...
Come back and reassign GPS to site level values: **This still needs to happen**
