...
```{r}
d <- getFormData("harvest", "Soil Sampling", "Soil Sampling 2017", forceUpdate = T)
...
```{r, results='asis'}
knitr::kable(iqrTab, row.names = F, digits = 0)
```
...
* Detect the type of dis  tribution
...
# outliers from log normal distribution. have to remove 0s to check
 ...
 lo = fitdistr((dat[,x])[complete.cases(dat[,x]) & dat[,x]!=0],"log-normal")
