...
```{r}
clustered_standard_errors <- function(dep.var, controls, dataset, id, returnconfit) {
  
  dep.var.col <- eval(as.name(dep.var), dataset)
  id.col <- eval(as.name(id), dataset)
  
  reg.formula <- as.formula(paste("dep.var.col ~ ", paste(controls, collapse="+"), sep = ""))
  
  reg <- lm(reg.formula, data=dataset)
  reg.covs <- cluster.vcov(reg, id.col)
  reg.clustered.errors <- coeftest(reg, reg.covs)
  
  # conf interval
  t<-qt(.90, reg$df.residual) # .90 level
  ct <- coeftest(reg, reg.covs)
  est<-cbind(ct[,1], ct[,1]-t*ct[,2], ct[,1]+t*ct[,2])
  colnames(est)<-c("Estimate","LowerCI","UpperCI")

  if (returnconfit == TRUE) {
    
    return(est)
    
  }
  
  return(reg.clustered.errors)
...
regFile <- "regFile_through2017.RData"
...
for(i in 1:length(modExport)){
  write.csv(modExport[i], file=paste0("output/ke2017/","regOutput_", i, ".csv"), row.names = T)
}
...
write.csv(finalModel, file="output/ke2017/indFe_ke2017.csv")
```
