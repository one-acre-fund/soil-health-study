...
ext.rw.ss <- extract(rw[, "NAME_2"], ss)
ss$spatialname <- ext.rw.ss$NAME_2

rwc <- intersect(ss, rw) # add polygons to the points

frw <- fortify(rw, region="NAME_2")
ss.soil <- aggregate(ss@data[,99:103], by=list(ss@data$spatialname), function(x){
  mean(x, na.rm=T)
})
...
Plot simplified soil map
```{r}
library(RColorBrewer)
for(i in 1:length(soilVars)){
print(
  ggplot(plotReady, aes(x=long, y=lat, group=group)) + geom_path() + 
