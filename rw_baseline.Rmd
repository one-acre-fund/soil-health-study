...
Let's look at the log results: See [here](http://www.cazaar.com/ta/econ113/interpreting-beta) for guidance on intepreting log transformed right hand side variables.
...
```
When we layer farmer perceived soil fertility on top of the principal component scatter plot, no clear pattern emergres. 
...
```{r}
plot(pca2)
ggplot(as.data.frame(pca2$x),aes(x=PC1,y=PC2)) + geom_point()
pca2$rotation
```
...
```{r}
psmVars <- paste(c("female", "age", "hhsize", "total.seasons", "own"),
                   collapse=" + ")
...
```{r}
# check for balance ala teffects
# check for difference in means and variance between variables >> see training
```
...
**Interpretation**: From an admittedly quick PSM exercise, we can find matches for a subset of the Tubura farmers and we see that we see no difference between the matched Tubura and control farmers on the pH outcome. We want to create matched groups and then assess the impact of Tubura duration and other covariates on soil health outcomes.
