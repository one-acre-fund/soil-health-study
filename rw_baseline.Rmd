---
title: "Rwanda Baseline Soil Health Study Report"
author: "Matt Lowes"
...
pdf(paste(od, "rw tenure vs ph.pdf", sep = "/"), width=11, height=8.5)
print(tenure.ph)
dev.off()
...
pdf(paste(od, "rw tenure vs fertilizer.pdf", sep = "/"), width=11, height=8.5)
...
### For Eric, CEC summary for new Ca and Mg thresholds
...
```{r}
write.csv(equal, file=paste(od, "rw female farmer status.csv", sep = "/"), row.names=T)
```

## Export the data for Eric  
  
```{r}
write.csv(d, file=paste(od, "rwanda_shs_merged_clean.csv",sep = "/"), row.names = F)
```
...
write.csv(thresh.t, file=paste(od, "table1_rw_thresholds_tenure.csv", sep = "/"), row.names = T)
```
...
write.csv(table1.a, file=paste(od, "psm coefficients ordered for ES-appendix.csv", sep = "/"))
...
write.csv(table2.a, file=paste(od, "psm coefficients ordered for ES_agprac-appendix.csv", sep = "/"))
...
write.csv(table3.a, file=paste(od, "psm coefficients table3-appendix.csv", sep = "/"))
```
...
I draw from [Robert's](robert.on@oneacrefund.org) example in the BGMS soil response notebook.
