...
list3 <- lapply(soilVars, function(x){
  mod <- lm(as.formula(paste("d[,x] ~",  "total.seasons", sep="")), data=d)
  return(mod)
})
...
list4 <- lapply(soilVars, function(x){
  mod <- lm(as.formula(paste("d[,x] ~",  logVars, "+ total.seasons", sep="")), data=d)
  return(mod)
})
...
previousSeason <- paste("fert_15b_log", "compost_15b_sqm_log", sep=" + ")
...
### Measured Yields
We don't have yield data for the Rwanda baseline so nothing to see here.
