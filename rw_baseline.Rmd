...
# leaflet(rw) %>% addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5)
# 
# leaflet(rw) %>% addTiles() %>%
#   setView(lng=rwanda$longitude, lat=rwanda$latitude, zoom=8) %>%
...
ggplot() + geom_histogram(data=subset(pr, pr$treat==1), aes(x = pr_score, y=..count.., fill=as.factor(treat)), bins=80, position = "identity") +
...
# match T and C farmers looking for only 1 match per treatment farmer
# set ties to FALSE because there are control observations in the dataset that tie in 
# terms of how well they match a treatment farmer. The resulting matched database is
# therefore much larger than the treatment data.
# I include match.out because I don't want the match to change each time I run the code
# IF I CHANGE THE MODEL HOWEVER, I NEED TO TAKE THAT OUT.
# I removed match.out=NULL 1/15

# the Match function uses the predicted likelihood of being in the treatment group from 
# reg to find nearest neighbor matches between the treatment and control groups. 

m <- Match(Y = y, Tr = tr, X = reg$fitted, ties=FALSE, replace=FALSE,
           caliper=0.25, estimand = "ATE")	
summary(m)
```

**Interpretation**: From an admittedly quick PSM exercise, we can find matches for a subset of the Tubura farmers and we see that we see no difference between the matched Tubura and control farmers on the pH outcome.
