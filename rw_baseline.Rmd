...
m <- lapply(psmList, function(listInput){
  ...
  # run glm regression:
  reg <- glm(as.formula(paste("client ~", listInput$psmVars, sep="")),  family= binomial(link="logit"), data=d)
  
  suppressWarnings(
  mod <- Match(Y = d[,listInput$y], Tr = listInput$tr, X = reg$fitted, ties=FALSE, replace=FALSE, caliper=0.25, estimand = "ATE")	
  )
  matchRes <- MatchBalance(tr ~ d[,listInput$y], match.out=mod, nboots=500, data=d, print.level = 0)
  return(list(mod, matchRes))
})
#lapply(m, summary)
```
...
coefTable <- do.call(rbind, lapply(1:length(m), function(model){
  ...
  res = data.frame(beta, pval)
  return(res)
}))
row.names(coefTable) <- y
...
stargazer(tenureTab, type="html", 
          ...
          covariate.labels = c("One Acre Fund Tenure"),
          dep.var.caption = "",
          dep.var.labels = "",
          column.labels = c(gsub("m3.","", soilVars)),
          notes = "Includes FE for cell",
