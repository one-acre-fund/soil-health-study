...
matchRes <- do.call(rbind, lapply(1:length(m), function(model){
  val <- as.data.frame(cbind(standard.diff=m[[model]][[2]]$AfterMatching[[1]]$sdiff, var.ratio = m[[model]][[2]]$AfterMatching[[1]]$var.ratio))
  return(val)
}))
rownames(matchRes) <- y
```
...
psmList <- list(
  list(tr = d$client,
       psmVars = paste(c("female", "age", "hhsize",
                         "cows", "goats", "chickens", "pigs", "sheep"),
                   collapse=" + "),
       y="m3.Ca"),
  list(tr = d$client,
...
m <- lapply(psmList, function(listInput){
  ...
  # run glm regression:
  reg <- glm(as.formula(paste("client ~", listInput$psmVars, sep="")),  data=d)
