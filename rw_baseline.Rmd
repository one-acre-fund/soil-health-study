...
for(i in 1:length(agPrac)){
  print(
    ggplot(d, aes(x=log10(d[,agPrac[i]]+1))) + geom_histogram(binwidth = 0.25) +
      labs(x = agPrac[i])
        )
}
...
maps <- lapply(soilVars, function(x){
  ...
  palColors <- colorNumeric(palette = "Reds", values(tps), na.color = "transparent")
...
stargazer(tenureTab, type="html", 
          ...
          #column.labels = c(gsub("m3.", "", as.character(namesInput))),
          notes = "Includes FE for cell",
