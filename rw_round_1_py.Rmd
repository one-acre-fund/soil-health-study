...
Try to calculate the point of diminishing returns for soil chemical values. The naive model would identify the point at which yield diminishes for just one soil parameter. A more advanced approach will use soil profiles. Let's start with a naive model and then sort out what this might look like 


```{r}
py$ph2 <- py$ph^2

test <- lm(tha ~ ph + ph2, data=py)

toGraph <- data.frame(resid = test$residuals, soil = py$ph[!is.na(py$ph)])

# looks just like the yield vs. ph plot
ggplot(toGraph, aes(x=soil, y=resid)) + 
  geom_point() + 
  stat_smooth(method="loess")

prime <- diff(py$tha)/diff(py$ph)
primeG <- data.frame(var1=c(NA,prime), var2=py$ph)

ggplot(primeG, aes(x=var2, y=var1)) + 
  geom_point() + 
  stat_smooth(method="loess")
