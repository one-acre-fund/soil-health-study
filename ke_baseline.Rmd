...
# Kgs of inputs per acre and hectare - Kalie/Charles?

**Scenario 1**: It's already per acre. This is not what the codebook suggests but it's possible?
...
However, if the input quatity variable is already per plot size, as the codebook suggests, I'd want to divide by the size of the plot to get the per acre application rate. 
...
d[which.max(d$dap.acre), c("inputs.dap_kg", "field_size", "dap.acre")]
...
```{r eval=F}
d$dap.ha <- (d$inputs.dap_kg/d$field_size)*2.47
d$can.ha <-(d$inputs.can_kg/d$field_size)*2.47
d$npk.ha <- (d$inputs.npk_kg/d$field_size)*2.47
d$urea.ha <- (d$inputs.urea_kg/d$field_size)*2.47

haInputs <- names(d)[grep(".ha", names(d))]

for(i in 1:length(haInputs)){
  print(
  ggplot(data=d, aes(x=d[,haInputs[i]])) +
    geom_density() +
    labs(x=haInputs[i], title = paste("Kenya input quantity per hectare - ", haInputs[i], sep = ""))
  )  
}
...
The larger values are definintely coming from the lower end of the field size spectrum. This suggests that the input quantities were always in acres and thus we should multiply the quantity by the field size to get the application rate for the field. However, the codebook says that the question ask for kgs used on the plot. 

**Kalie/Charles** >>> can you please clarify?
...
"fertilizer_5", "legum_maincrop_5", "legum_intercrop_5", soilVars, "valley", "hilltop", "hillside", "crop.maize", "alt")
...
t12order <- c("cows", "pigs", "sheep", "goats", "chickens")
...
soilVars <- c("Mg", "pH", "Ca", "Exch.Al", "C.E.C")
...
ke.table17 <- ke.table17[,c("pH", "Ca", "Mg", "Exch.Al", "C.E.C")]
...
```{r eval=F}
ke.table14 <- do.call(cbind, lapply(list2, function(x){
  plm.log(x, 2)
}))
```
