...
for(i in 1:length(acreInputs)){
print(
    ggplot(d, aes(x = field_size.adj, y=d[, acreInputs[i]])) + 
    geom_point() + 
    #facet_wrap(~oaf) +
    labs(x = "Field Size in Acres", y=acreInputs[i], title = paste("Kenya input quantity by acreage - ", acreInputs[i], sep = ""))
)
...
table18 <- table18[,c("pH", "Ca", "Mg", "Exch.Al")]
...
table18b <- table18b[,c("pH", "Ca", "Mg", "Exch.Al")]
...
table18c <- table18c[,c("pH", "Ca", "Mg", "Exch.Al")]
