...
"fertilizer_5", "legum_maincrop_5", "legum_intercrop_5", soilVars, "valley", "hilltop", "hillside", "crop.maize")
...
```{r}
# This is currently only for OAF clients!
list1 <- lapply(soilVars, function(x){
  mod <- lm(as.formula(paste("oafOnly[,x] ~",  logVars, "+ as.factor(SiteName)", sep="")), data=oafOnly)
  return(mod)
})
...
```{r}
psmVars <- paste(out.list <- c("female", "age", "cows", "goats", "chickens","pigs", "sheep"), collapse=" + ")
...
coreVars = c("female", "age", "as.factor(site)", "cows", "goats", "chickens", "pigs", "sheep")
