---
title: "Kenya Baseline Demographic and Soil Summary and Analysis"
author: "Matt Lowes"
...
```{r eval=FALSE}
psmVars <- paste(out.list <- c("female", "age", "seasons_oaf", "cows", "goats", "chickens","pigs", "sheep", "field_size", "yield",  "chemical_fertilizer_5", "compost_fertilizer_5", "lime_fertilizer_5", "fertilizer_5", "legum_maincrop_5", "legum_intercrop_5"), collapse=" + ")

# inputs.dap_kg", "inputs.can_kg", "inputs.npk_kg", "inputs.urea_kg", "inputs.dap_kg_intercrop", "inputs.can_kg_intercrop", "inputs.npk_kg_intercrop","inputs.urea_kg_intercrop",
...
toRemove <- c("seasons_oaf")
# so few control farmesr have OAF experience this strongly distorts the Pr() match
psmCheck <- psmCheck[!psmCheck %in% toRemove]

psmVars <- paste(psmCheck, collapse=" + ")

reg <- glm(as.formula(paste("oaf ~", psmVars, sep="")),  data=h)
summary(reg)	
...
# PSM prep
tr <- cbind(h$oaf)
x <- h[, unlist(strsplit(psmVars, " + ", fixed=T))]
y <- soilVars
#y <- cbind(d$pH)
...
m <- lapply(y, function(response){
  ...
  suppressWarnings(
  mod <- Match(Y = h[,response], Tr = tr, X = reg$fitted, ties=FALSE, replace=FALSE, caliper=0.25, estimand = "ATE")	
  )

  return(list("output"=mod, "df"=rbind(h[mod$index.treated,], h[mod$index.control,])))
})
```
